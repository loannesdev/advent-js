...
> [!IMPORTANT]
> Solución realizada con ayuda de Copilot de Windows 11 (chatGPT)
...
function getMaxGifts(
  ...
  function findMaxSum(currentSum: number, citiesVisited: number, index: number) {
    if (citiesVisited > maxCities || currentSum > maxGifts) {
      return;
    }
    ...
    for (let i = index; i < giftsCities.length; i++) {
      findMaxSum(currentSum + giftsCities[i], citiesVisited + 1, i + 1);
    }
  }
