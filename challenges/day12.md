...
La dificultad del reto está en pensar que sólo podemos configurar el salto del trineo una vez y que buscamos el salto mínimo que nos serviría para sortear todos los obstaculos. <br ><br >

## **Solución**

```js
export default function getMinJump(obstacles) {
  const MAX_VALUE = Math.max(...obstacles) + 1;
  const ARR = Array.from({ length: MAX_VALUE })
    .map((_, index) => index)
    .slice(1);

  const AUX = ARR.filter((elm) => !obstacles.some((v) => elm === v));

  for (let x of AUX) {
    let LAST_VALIDATION = null;

    for (let i = 0; i <= ARR.length; i++) {
      const LAST_CANDIDATES = AUX.length > 2 ? x * (i + 1) : x;
      LAST_VALIDATION = !obstacles.some((k) => k === LAST_CANDIDATES);

      if (!LAST_VALIDATION) break;
    }

    if (LAST_VALIDATION) return x;
  }
}
```