...
export default function countPackages(carriers, carrierID) {
  ...
  carriers.forEach(([carrier, packs, subs]) => {
    Object.assign(obj, {
      [carrier]: {
        packs,
        subs,
  ...
  const { packs, subs } = obj[carrierID];
  let totalNumPackages = packs;

  const recursiveFun = (arr) => {
    arr.forEach((v) => {
      const { packs, subs } = obj[v];

      totalNumPackages += packs;
      if (subs.length) recursiveFun(subs);
    });
  };

  recursiveFun(subs);

  return totalNumPackages;
}
```