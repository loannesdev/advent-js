...
export default function canCarry(capacity, trip) {
  ...
  for (let i = 0; i < mapedTrips.length; i++) {
    ...
    const next = mapedTrips[i + 1];

    if (next && next.value === element.value) packages = 0;
    if (element.flag === "in") packages += element.packs;
    if (element.flag === "out") packages -= element.packs;
    if (packages > capacity) return false;
  }
