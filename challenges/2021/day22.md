...
export default function countDecorations(tree) {
  ...
  const checkDecorations = ({ value, left, right })=>{
    totalDecorations += value;
    
    if (left && left instanceof Object) checkDecorations(left);
    if (right && right instanceof Object) checkDecorations(right);
  }

  checkDecorations(tree);
